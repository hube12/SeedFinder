...
int main(int argc, char *argv[]) {
    if (argc!=3){
        printf("Input is <finder_name>.exe RegX RegZ\n");
        return -1;
    }
    // Translate the positions to the desired regions.
        int regPosX = (int)strtol(argv[1], NULL, 0);
        int regPosZ = (int)strtol(argv[1], NULL, 0);
    ...
    int *biomeCache = allocCache(lFilterBiome, 256, 256);
...
*/          
            ...
            if (areBiomesViable(we,maps,posSpawn.x,posSpawn.z,400,isMoo)){
