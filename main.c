...
void MansionCoordinateFinder(long x, long z, int64_t seed) {
    initBiomes();
    LayerStack g = setupGenerator(MC_1_12);
    int *map = allocCache(&g.layers[g.layerNum - 1], 32, 32);
    applySeed(&g, seed);
    int regX=(int) (x/80/16);
    int regZ=(int) (z/80/16);
    printf("%d %d\n",regX,regZ);
    int distance=10;
    for (int i = -distance; i < distance; ++i) {
        for (int j = -distance; j < distance; ++j) {
            if (isViableMansion(&g,map,seed,regX+i,regZ+j)){
                printf("e\n");
                Pos pos= getLargeStructurePos(MANSION_CONFIG,seed,regX+i,regZ+j);
                printf("%d %d \n",pos.x,pos.z);
            }
        }
...
int main(int argc, char *argv[]) {
    if (argc != 4) {
        ...
        MansionCoordinateFinder(252313, -97344,0);
    } else {
